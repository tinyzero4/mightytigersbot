...
import { SchedulerService } from './services/schedule-service';
...
connection.then((db) => {
  ...
  bot.command('/newteam', ({ reply, chat }) => {
    teamService.create(new Team(chat.title, chat.id))
      .then(team => chatService.sendGreeting(reply))
          ...
          teamService.nextMatch(chat.id).then(([match, created]) => {
            if (match && created)
              chatService.sendMatchVoteMessage(replyWithHTML, matchService.matchStats(match))
                ...
                .then(message_id => matchService.setMatchMessage(match._id, data.message_id))
                .catch(err => onError(err, reply))
          }).catch(err => onError(err, reply))
      })
  });

...
export class MatchService {
    ...
    constructor(scheduleService: SchedulerService) {
     ...
     */
    async nextMatch(team: Team): Promise<[Match, boolean]> {
        if (!team) return [undefined, false];

        const latestMatch = await this.findLatest(team.team_id);
        const now = new Date();
        if (latestMatch) {
            if (latestMatch.status == MatchStatus.SCHEDULED && !MatchService.hasMatchStarted(latestMatch, now)) {
                return [latestMatch, false];
            } else {
                this.completeMatch(latestMatch);
            }
        }

        const match = await this.scheduleNextMatch(team);
        return [match, !!match];
    }

    private static hasMatchStarted(match: Match, now: Date): boolean {
    ...
    /**
     * Builds details information with stats for given match. Used in chat conversation logic.
     * @param match - match
     */
    resolveMatchDetails(match: Match): any {
        let playingTotal = 0;
    ...
    validateConfirmation(c: ConfirmationEvent): Promise<boolean> {
        return this.find(c.matchId).then(m => {
            if (!m || m.completed || MatchService.hasMatchStarted(m, new Date())) return false;
    ...
    cancelObsoleteMatches(team_id): Promise<boolean> {
        return this.findLatest(team_id).then(match => {
            if (match && !match.completed && !MatchService.hasMatchStarted(match, new Date()))
    ...
    private create(match: Match): Promise<InsertOneWriteOpResult> {
        return this.matchColl.then(c => c.insertOne({
            ...match,
            createdAt: new Date(),
            squad: {},
            withMe: {},
